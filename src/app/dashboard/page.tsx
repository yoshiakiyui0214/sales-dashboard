...
function KpiCard({
  ...
  return (
    <div className="bg-white rounded-lg border-l-4 border-l-[#1A2E5C] shadow-sm p-6">
      <p className="text-sm text-zinc-600">{label}</p>
      <p className="text-3xl font-bold text-[#1A2E5C] mt-1">{value}</p>
...
export default async function DashboardPage() {
  ...
  if (records.length === 0) {
    return (
      <main className="min-h-screen bg-[#F8FAFC] p-8">
        <h1 className="text-2xl font-bold text-[#1A2E5C] mb-4">
          売上分析ダッシュボード
        </h1>
        <p className="text-zinc-600">
          まだデータがありません。先に
          <a href="/upload" className="text-[#1A2E5C] underline mx-1">
            CSVをアップロード
          </a>
          してください。
        </p>
    ...
    <main className="min-h-screen bg-[#F8FAFC] p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-lg border p-6 mb-8">
          <h2 className="font-semibold text-[#1A2E5C] mb-4">月次売上推移</h2>
         <RevenueChart data={kpi.monthlyData} />
        </div>

        {/* カテゴリ別売上 */}
        <div className="bg-white rounded-lg border p-6 mb-8">
          <h2 className="font-semibold text-[#1A2E5C] mb-4">カテゴリ別売上</h2>
        ...
        <div className="bg-white rounded-lg border p-6">
          <h2 className="font-semibold text-[#1A2E5C] mb-4">
            SKUトップ10(数量ベース)
          </h2>
          <ul className="space-y-2">
